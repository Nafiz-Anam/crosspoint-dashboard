        ...
        try {
          ...
          if (
            ...
            return {
              ...
              branchId: user.branchId, // Include branchId from backend
    ...
    async jwt({ token, user }) {
      ...
      if (token.refreshToken && token.refreshTokenExpires && Date.now() < token.refreshTokenExpires) {
        try {
          ...
          } else if (refreshData.accessToken && refreshData.expires_in) {
            // If flat with expires_in
            token.accessToken = refreshData.accessToken
            token.accessTokenExpires = Date.now() + refreshData.expires_in * 1000
          } else {
            // Fallback if refresh API response structure is unexpected
